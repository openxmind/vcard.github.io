...
class VcardRepository extends BaseRepository
{
    ...
    public function getDuplicateVcard($vcard)
    {
        ...
        $matchAlias = Vcard::where('url_alias', 'LIKE', '%'.$baseAlias.'%')->get();
        foreach ($matchAlias as $alias) {
            $aliasLastCharCheck = str_replace($baseAlias, '', $alias->url_alias);
            $lastCharArr[] = $aliasLastCharCheck;
        }
        $maxChar = max($lastCharArr);
        $maxChar++;
        $newVcard->url_alias = $baseAlias.$maxChar;
        $newVcard->save();
        ...
        foreach ($vcard->subscriptions as $newVcardSubscription) {
            $newVcardSubscription = $newVcardSubscription->replicate();
            $newVcardSubscription->tenant_id = $newVcard->tenant_id;
            $newVcardSubscription->save();
        }
